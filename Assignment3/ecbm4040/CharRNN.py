...
class CharRNN():
    ...
    def rnn_layer(self):
        ...
        #cite: https://github.com/GarrettHoffman/lstm-oreilly/blob/master/Stock%20Market%20Sentiment%20with%20LSTMs%20and%20TensorFlow.ipynb
        ...
        if cell_type == 'LSTM':
            for i in range(num_layers):
                c = tf.nn.rnn_cell.LSTMCell(rnn_size)
                c = tf.nn.rnn_cell.DropoutWrapper(c, output_keep_prob=keep_prob)
                cell.append(c)
        elif cell_type == 'GRU':
            for i in range(num_layers):
                c = tf.nn.rnn_cell.GRUCell(rnn_size)
                c = tf.nn.rnn_cell.DropoutWrapper(c, output_keep_prob=keep_prob)
                cell.append(c)
        ...
        outputs, final_state = tf.nn.dynamic_rnn(cell, rnn_inputs, initial_state=self.initial_state)
    ...
    def my_optimizer(self):
        ...
        # cite: https://stackoverflow.com/questions/36498127/how-to-apply-gradient-clipping-in-tensorflow
        
        global_step = tf.Variable(0, trainable=False, name='global_step')
        optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate)
        # 使用clipping gradients
        tvars = tf.trainable_variables()
        grads, _ = tf.clip_by_global_norm(tf.gradients(ys=self.loss, xs=tvars),clip_norm=self.grad_clip)
    ...
    def sample(self, checkpoint, n_samples, vocab_size, vocab_to_ind, ind_to_vocab, prime='You \n'):
        ...
        # cite: https://github.com/sherjilozair/char-rnn-tensorflow/blob/master/model.py
        self.session = tf.Session()
        with self.session as sess:
            ...
            for char in samples:
                x = np.zeros((1, 1))
                x[0, 0] = vocab_to_ind[char]
                feed = {self.inputs: x, self.keep_prob: 1.0, self.initial_state: state}
            ...
            for _ in range(n_samples):
                ...
                x[0, 0] = vocab_to_ind[char]
                feed = {self.inputs: x, self.keep_prob: 1.0, self.initial_state: state}
                [probs,state] = sess.run([self.prob_pred,self.final_state], feed)
                p = pick_top_n(probs, vocab_size,top_n=5)
                phrase += ind_to_vocab[p]
