...
class SGDmomentumOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        ###################################################
        for k in grads: # for each gradient
            # calculate new velocity as the momentum times the old plus the learning rate times the new
            velocitys[k] = momentum * velocitys[k] + learning_rate * grads[k]
            params[k] -= velocitys[k] # subtract velocity from the params
...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        ###################################################
        for k in grads: # for each axis
            # calculate the cache value as a function of the past squared gradients, and the square of the current
            cache[k] = gamma*cache[k] + (1-gamma)*grads[k]**2
            # evaluate the parameter update as a function of the new grad over the cache (old) values
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads: #for each axis
            t += 1 # incriment time
            momentums[k] = beta1*momentums[k] + (1-beta1)*grads[k] # find momentum
            mt = momentums[k] / (1-beta1**t) # find updated momentum
            velocitys[k] = beta2*velocitys[k] + (1-beta2)*(grads[k]**2) #find velocity
            vt = velocitys[k] / (1-beta2**t) #find updated velocity
            params[k] -= learning_rate * mt / (np.sqrt(vt) + eps) # update parameters
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads: # for each axis
            t += 1 # update timestep
            momentums[k] = beta1*momentums[k] + (1-beta1)*grads[k] # find new momentum
            mt = momentums[k] / (1-beta1**t) # update based on beta
            velocitys[k] = beta2*velocitys[k] + (1-beta2)*grads[k]**2 # find velocity
            vt = velocitys[k] / (1-beta2**t) # update based on bata
            params[k] -= learning_rate / (np.sqrt(vt) + eps) * (beta1 * mt + (1-beta1)*grads[k]/(1-beta1**t)) #update parameters