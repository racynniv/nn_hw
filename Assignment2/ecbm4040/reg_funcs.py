...
def dropout_forward(x, dropout_config, mode):
    ...
    if mode == "train":
        ...
        # backward.                               #
        ###########################################
        cache = np.random.binomial(1,keep_prob,size=x.shape) # set cache as a binary of which are kept and which arent
        out = x*cache # multiply the values by the binary cache
        ...
        ##########################################
        out = x*keep_prob # for training, send out averaged signals
        cache = None
...
def bn_forward(x, gamma, beta, bn_params, mode):
    ...
    if mode == "train":
        ...
        #############################################################
        mean = np.mean(x,axis=0) # find the mean
        var = np.var(x,axis=0) # and the var
        norm = (x-mean)/np.sqrt(var+eps) # find the norm for each value in x
        out = gamma*norm + beta # get the output value by multiplying by slope and adding B
        moving_mean = decay*moving_mean + (1-decay)*mean # evaluate new moving mean
        moving_var = decay*moving_var + (1-decay)*var # evaluate new moving var
        ...
        #############################################################
        mean = moving_mean # set mean to moving mean
        var = moving_var # set var to moving var
        norm = (x-mean)/np.sqrt(var+eps) # find normalized value for each x
        out = gamma*norm + beta # set output variable
